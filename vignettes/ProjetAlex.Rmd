...
```{r}
#Here, we are going to use a separate function I coded which gives us the list of best/worst ranking countries for a given year and a given criteria
criteria_analysis('Economy', 10, 2017)
```

This is interesting, because those countries are not even in the first 20 countries in the GDP per capita ranking (except for Switzerland which is 17th). 

This is kind of obvious, but we now know that GDP per capita and GDP per capita actual contribution to happiness are two very different things.
